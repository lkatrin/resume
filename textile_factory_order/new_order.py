...
class Ui_Form(QWidget):
    ...
    def create_order(self):
        ...
        try:
            # Получаем ID менеджера из user_data
            manager_id = self.user_data['id']  # Используем ID пользователя как ID менеджера
            print(f"[DEBUG] Используем ID менеджера из user_data: {manager_id}")

            # Подготовка данных для заказа
            customer_name = self.customer_combobox.currentText()
            print(f"[DEBUG] Выбранный заказчик: {customer_name}")

            if customer_name in self.company_map:
                customer_id = self.company_map[customer_name]
                print(f"[DEBUG] Заказчик является компанией. ID: {customer_id}")
            else:
                customer_id = self.person_map[customer_name]
                print(f"[DEBUG] Заказчик является физическим лицом. ID: {customer_id}")
            ...
            for row in range(self.products_table.rowCount()):
                ...
                if not product_type_id:
                    error_message = f"Тип продукции '{product_type}' не найден в базе данных."
                    print(f"[ОШИБКА] {error_message}")
                    QtWidgets.QMessageBox.warning(self, "Ошибка", error_message)
                    return
                ...
                # Материалы (ткань, нитки, фурнитура, декор, подкладка)
                materials = [
    ...
    def add_order_to_db(self, worker_id, customer_id, products):
        ...
        try:
            with self.db.cursor() as cursor:
                ...
                for product in products:
                    ...
                    cursor.execute(product_query, (
                        ...
                        0.00,  # real_price_of_one (по умолчанию)
                        1  # making_stage_id = 1 (нач. этап)
                    ))
                    product_id = cursor.lastrowid  # ID созданного изделия
                    ...
                    for material in product["materials"]:
                        ...
                        cursor.execute(material_query, (
                            ...
                            None,  # Ширина (по умолчанию 0)
                            None  # Длина (по умолчанию 0)
                        ))
            ...
            # Откат изменения в случае ошибки
            self.db.rollback()
    ...
    def get_material_id(self, material_name):
        ...
        try:
            with self.db.cursor() as cursor:
                ...
                result = cursor.fetchone()
                if result:
                    # Извлечение ID материала из словаря
    ...
    def load_materials(self, category_id):
        """Загрузка материалов по категории"""
        if not self.db:
            return []
    ...
    def load_product_types(self):
        """Загрузка типов продукции с описанием"""
        if not self.db:
            return []
    ...
    def setupUi(self, Form):
        ...
        table_scroll.setWidgetResizable(True)  # Разрешаем изменение размера
        ...
        # Заказчиков в выпадающий список
        self.customer_combobox = QComboBox()
        customers, self.company_map, self.person_map = self.load_customers()
        self.customer_combobox.addItems(customers)
        content_layout.addWidget(QLabel("Выберите заказчика"))
        content_layout.addWidget(self.customer_combobox)
    ...
    def create_input_fields(self, layout):
        ...
        self.fields = {
            ...
            "Нитки": QComboBox(),
            "Фурнитура": QComboBox(),
            "Количество фурнитуры": QLineEdit(),  # Новое поле
            "Декор": QComboBox(),
            "Количество декора": QLineEdit(),  # Новое поле
            "Подкладка": QComboBox(),
    ...
    def toggle_hardware_quantity_field(self, text):
        ...
        if text != "-":
            ...
            self.fields["Количество фурнитуры"].setVisible(False)
            self.fields["Количество фурнитуры"].clear()  # Очищаем поле, если фурнитура не выбрана
    ...
    def toggle_decor_quantity_field(self, text):
        ...
        if text != "-":
            ...
            self.fields["Количество декора"].setVisible(False)
            self.fields["Количество декора"].clear()  # Очищаем поле, если декор не выбран
    ...
    def add_to_order(self):
        ...
        # Невозможность добавления другого заказчика на тот же заказ
        if self.current_customer_id is not None and self.current_customer_id != customer_id:
            QtWidgets.QMessageBox.warning(None, "Ошибка",
                                          "Нельзя добавлять изделия для разных заказчиков в один заказ.")
            return

        # Если заказчик еще не был выбран, сохраняем его ID
        ...
        # Форматирование в формат "название - количество"
        ...
        if data[7] != "-" and data[8] != "-":  # Если декор выбран
            data[7] = f"{data[7]} - {data[8]}"  # Объединяем название и количество

        # Убираем отдельные столбцы для количества фурнитуры и декора
        ...
        # Скрытие полей для количества фурнитуры и декора
        self.fields["Количество фурнитуры"].setVisible(False)
        self.fields["Количество декора"].setVisible(False)
